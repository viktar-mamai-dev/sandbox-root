 ...
 * Copyright (c) 2023
 */
package com.mamay.patterns.blinov.behave;
    ...
    switch (cmd) {
      case CREDITING:
        if (account.isBlocked()) {
          out.println("Sorry, the crediting account #" + account.getId() + " is blocked!");
        } else {
      ...
      case WITHDRAWING:
        if (account.isBlocked()) {
          out.println("Sorry, the withdraw account#" + account.getId() + " is blocked!");
        } else {
          double balance = account.getBalance() - amount;
          account.setBalance(balance);
          out.println(amount + " is withdrawed from the account #" + account.getId());
        }
        break;
      case BLOCKING:
        if (account.isBlocked()) {
