...
public class AquaContainer {
  ...
  public Optional<Component> getPossibleComponent(Item name) {
    return componentList.stream().filter(component -> component.getName().equals(name)).findAny();
  }

  public Set<Item> getComponentNames() {
    return this.componentList.stream().map(Component::getName).collect(Collectors.toSet());
  }

  public boolean isFullAquarium(Aquarium aqua) {
    Set<Item> names = this.getComponentNames();
    return aqua.getComponents().stream().map(Component::getName).allMatch(names::contains);
  }
}
