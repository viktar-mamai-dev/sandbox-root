...
public interface QuerySQL {
  ...
  String FIND_ALL_MAGAZINES =
      "SELECT * FROM magazine m INNER JOIN period p" + " ON m.period_id=p.period_id";
  ...
  String CREATE_MAGAZINE =
      "INSERT INTO magazine" + "(title, annotation, period_id, location) VALUES(?, ?, ?, ?)";
  String UPDATE_MAGAZINE =
      "UPDATE magazine SET title=?, annotation=?," + " period_id=? WHERE magazine_id=?";
  String MAGAZINE_COUNT = "SELECT COUNT(*) AS count FROM magazine";

  // SUBSCRIPTION QUERIES
  String FIND_ALL_SUBS =
      "SELECT * FROM subscription s INNER JOIN magazine m"
  ...
  String CREATE_SUB =
      "INSERT INTO subscription" + "(magazine_id, sub_index, duration, price) VALUES(?, ?, ?, ?)";
  String UPDATE_SUB =
      "UPDATE subscription SET sub_index=?, duration=?," + " price=? WHERE subscription_id=?";

  // RESERVATION QUERIES
  String FIND_ALL_RESERV =
  ...
  String CREATE_RESERV =
      "INSERT INTO reservation" + " (user_id, subscription_id, count) VALUES(?, ?, ?)";
