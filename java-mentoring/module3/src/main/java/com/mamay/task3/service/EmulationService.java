...
public class EmulationService {
    ...
    public void start() throws InterruptedException {
        ...
        Long winner = raceService.retrieve(currentEmulation.getRaceId()).getParticipants().get(indexForWinner);
    ...
    public void doProgress(Emulation emulation) throws InterruptedException {
        ...
        IntStream.range(0, race.getPositions().size()).forEach(i -> race.changePositions(i, LOWER_PACE + random.nextDouble() * 10));
    ...
    private void logCurrentState(Emulation emulation) {
        ...
        for (Horse horse : horses) {
            out.println("Horse " + horse.getName() + " with rider " + horse.getRider().getName() + " at position "
