 ...
 * positive integers in two different ways: a^3 + b^3 = c^3 + d^3 Design an algorithm to find all
...
public class Task3 {
  public List<Long> getTaxiCabList(int n) {
    ...
    while (!pq.isEmpty()) {
      ...
      if (curr.j < n) pq.insert(new Node(curr.i, curr.j + 1));
    }
    if (run > 1) StdOut.println();
  ...
  class Node implements Comparable<Node> {
    ...
    public int compareTo(Node that) {
      ...
      else return Integer.compare(this.i, that.i);
    }
