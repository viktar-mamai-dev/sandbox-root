...
public class ListSetTest extends TestCase {
    ...
    private static TestResultsPair runKernel(final ListFactory factoryA,
        ...
        for (int r = 0; r < 5; r++) {
            ...
            TestResults newResults = mainKernel(numThreads, factoryA.construct(), lblA, addSeq, containsSeq, removeSeq);
            ...
            newResults = mainKernel(numThreads, factoryB.construct(), lblB, addSeq, containsSeq, removeSeq);
    ...
    private static TestResults mainKernel(final int numThreads,
        ...
        while (curr != null) {
            assertTrue("List was not sorted, index " + (listLengthAfterAdds - 1) + " is " +
                            prev.object + " and index " + listLengthAfterAdds + " is " + curr.object,
        ...
        final double containsRate = (double) (numThreads * containsSeq.sequenceLength()) / (double) containsTime;
        final double removeRate = (double) (numThreads * removeSeq.sequenceLength()) / (double) removeTime;
