...
public class ReciprocalArraySumTest extends TestCase {
  ...
  private double parTestHelper(final int N, final boolean useManyTaskVersion, final int ntasks) {
    ...
    TestResults<Double> seqResults =
        testExecutor.execute(() -> ReciprocalArraySum.seqArraySum(input));
    ...
    DoubleSummaryStatistics seqStat =
        seqResults.getResults().stream().mapToDouble(d -> d).summaryStatistics();
    DoubleSummaryStatistics parStat =
        parResults.getResults().stream().mapToDouble(d -> d).summaryStatistics();
