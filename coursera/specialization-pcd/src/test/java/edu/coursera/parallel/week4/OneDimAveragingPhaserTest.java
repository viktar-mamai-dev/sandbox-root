...
public class OneDimAveragingPhaserTest extends TestCase {
  ...
  private static final int niterations = 40000;
  ...
  private double parTestHelper(final int N, final int ntasks) {
    ...
    TestResults<double[]> parFuzzyResults =
        testExecutor.execute(
            () ->
                OneDimAveragingPhaser.runParallelFuzzyBarrier(
                    niterations, myNew, myVal, N, ntasks));
  ...
  public void testFuzzyBarrier() {
    ...
    final String errMsg =
        String.format(
            "It was expected that the fuzzy barrier parallel implementation would "
                + "run %fx faster than the barrier implementation, but it only achieved %fx speedup",
            expected, speedup);
    assertTrue(errMsg, speedup >= expected);
    final String successMsg =
        String.format(
            "Fuzzy barrier parallel implementation "
                + "ran %fx faster than the barrier implementation",
            speedup);
