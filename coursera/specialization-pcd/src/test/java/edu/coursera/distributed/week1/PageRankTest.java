...
public class PageRankTest extends TestCase {
  ...
  private static JavaPairRDD<Integer, Website> generateGraphRDD(
    ...
    return context
        ...
        .mapToPair(
            i ->
                new Tuple2<Integer, Website>(
                    i, generateWebsite(i, nNodes, minEdgesPerNode, maxEdgesPerNode, edgeConfig)));
  }
  ...
  static void testDriver(
      ...
      final EdgeDistribution edgeConfig) {
    System.err.println(
        "Running the PageRank algorithm for "
            + niterations
            + " iterations on a website graph of "
            + nNodes
            + " websites");
    System.err.println();
    ...
    Function<Integer, List<Tuple2<Integer, Double>>> func =
        (cores) ->
            runPageRank(
                nNodes, minEdgesPerNode, maxEdgesPerNode, niterations, edgeConfig, repeats, cores);

    TestResults<List<Tuple2<Integer, Double>>> pageRankResults = testExecutor.execute(1, func);
    TestResults<List<Tuple2<Integer, Double>>> pageRankMultipleResults =
        testExecutor.execute(nCores, func);
    ...
    double singleElapsed = pageRankResults.getExecutionTime(),
        parElapsed = pageRankMultipleResults.getExecutionTime();
    double speedup = singleElapsed / parElapsed;
    System.err.println("Single-core execution ran in " + singleElapsed + " ms");
    System.err.println(
        nCores
            + "-core execution ran in "
            + parElapsed
  ...
  private static List<Tuple2<Integer, Double>> runPageRank(
      int nNodes,
      int minEdgesPerNode,
      int maxEdgesPerNode,
      int niterations,
