...
public class MpiJavaTest extends TestCase {
    ...
    private void testDriver(final int M, final int N, final int P) throws MPIException {
        ...
        if (myrank == 0) {
            System.err.println("Testing matrix multiply: [" + M + " x " + N + "] * [" + N + " x " + P + "] = ["
                    + M + " x " + P + "]");
        }

        TestResults<Matrix> seqResults = testExecutor.execute(() -> {
        ...
        TestResults<Matrix> parResults = testExecutor.execute(() -> {
        ...
        if (myrank == 0) {
            ...
            for (int i = 0; i < parMatrix.getNRows(); i++) {
                for (int j = 0; j < parMatrix.getNCols(); j++) {
                    final String msg = "Expected " + seqMatrix.get(i, j) + " at (" + i + ", " + j + ") but found "
                            + parMatrix.get(i, j);
