...
public class HttpServer {
    ...
    public HttpResponse sendHttpRequest(final String path, final boolean print) throws IOException {
        ...
        if (print) {
            System.err.println("Response code is " + responseCode + ", with content length " + responseStr.length());
        }
    ...
    public void terminate() throws IOException, InterruptedException {
        System.err.println("Stopping server for " + Thread.currentThread().getStackTrace()[2].getMethodName());
        socket.close();
        thread.interrupt();
        thread.join();
    }
