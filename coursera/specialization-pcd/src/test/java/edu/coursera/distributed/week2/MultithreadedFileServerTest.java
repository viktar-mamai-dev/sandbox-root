...
public class MultithreadedFileServerTest extends BaseFileServerTest {
  ...
  private int runPerformanceTest(BaseFileServer baseServer)
  ...
  public void testPerformance() throws IOException, InterruptedException {
    final int nDriverThreads = baseServer.getnCores();
    System.err.println(
        "Testing performance of multi-threaded web server using "
            + nDriverThreads
            + " request threads");
    ...
    final String msg =
        "Expected parallel threads to produce at least a "
            + expected
            + "x improvement, but only saw "
            + improvement
            + "x";
