...
/**
 * Wrapper class for all supported Java MPI APIs. For more detailed documentation of the wrapped MPI
 * APIs, refer to https://www.open-mpi.org/doc/v1.8/
 */
public final class MPI {
   ...
   * @param count Number of elements in buf to perform bcast with, starting at offset
   * @param root Root rank of this broadcast
   * @param comm Communicator to use
   * @throws MPIException On MPI error
   */
  public void MPI_Bcast(
      final double[] buf, final int offset, final int count, final int root, final MPI_Comm comm)
      throws MPIException {
  ...
  public void MPI_Send(
    ...
    final int err =
        MPILib.INSTANCE.MPI_Send(wrapper, count, MPI_INTEGER.datatype, dst, tag, comm.comm);
    if (err != 0) {
      throw new MPIException(err);
    }
  }
  ...
  public void MPI_Send(
    ...
    final DoubleBuffer wrapper = DoubleBuffer.wrap(buf, offset, count);
    final int err =
        MPILib.INSTANCE.MPI_Send(wrapper, count, MPI_DOUBLE.datatype, dst, tag, comm.comm);
    if (err != 0) {
      throw new MPIException(err);
    }
  }
  ...
  public void MPI_Recv(
    ...
    final IntBuffer wrapper = IntBuffer.wrap(buf, offset, count);
    final int err =
        MPILib.INSTANCE.MPI_Recv(
            wrapper, count, MPI_INTEGER.datatype, src, tag, comm.comm, MPI_STATUS_IGNORE.status);
    if (err != 0) {
      throw new MPIException(err);
    }
  }
  ...
  public void MPI_Recv(
    ...
    final int err =
        MPILib.INSTANCE.MPI_Recv(
            wrapper, count, MPI_DOUBLE.datatype, src, tag, comm.comm, MPI_STATUS_IGNORE.status);
    if (err != 0) {
      throw new MPIException(err);
    }
  }
  ...
  public MPI_Request MPI_Irecv(
    ...
    final int err =
        MPILib.INSTANCE.MPI_Irecv(
            request.buf, count, MPI_DOUBLE.datatype, src, tag, comm.comm, request.request);
    if (err != 0) {
      throw new MPIException(err);
    }
    return request;
  ...
  public MPI_Request MPI_Isend(
    ...
    final int err =
        MPILib.INSTANCE.MPI_Isend(
            request.buf, count, MPI_DOUBLE.datatype, dst, tag, comm.comm, request.request);
    if (err != 0) {
      throw new MPIException(err);
    }
    return request;
