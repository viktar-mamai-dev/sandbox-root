...
public final class StudentAnalytics {
    ...
    /**
     * TODO compute the average age of all actively enrolled students using parallel streams. This should mirror
     * the functionality of averageAgeOfEnrolledStudentsImperative. This method should not use any loops.
     *
     * @param studentArray Student data for the class.
     * @return Average age of enrolled students
     */
    public double averageAgeOfEnrolledStudentsParallelStream(final Student[] studentArray) {
        final double DEFAULT_IF_EMPTY = 0d;
        OptionalDouble average = Arrays.stream(studentArray).parallel()
                .filter(Student::checkIsCurrent)
                .mapToDouble(Student::getAge)
                .average();
        return average.orElse(DEFAULT_IF_EMPTY);
    }

    /**
     * Sequentially computes the most common first name out of all students that are no longer active in the
     * class using loops.
     *
     * @param studentArray Student data for the class.
    ...
    /**
     * TODO compute the most common first name out of all students that are no longer active in the class using
     * parallel streams. This should mirror the functionality of mostCommonFirstNameOfInactiveStudentsImperative.
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(final Student[] studentArray) {
        final String DEFAULT_IF_EMPTY = null;
        Predicate<Student> isCurrent = Student::checkIsCurrent;
        final Map<String, Long> map = Arrays.stream(studentArray).parallel()
                .filter(isCurrent.negate())
                .collect(Collectors.groupingBy(Student::getFirstName, Collectors.counting()));

        Optional<String> maxUsername = map.keySet().parallelStream()
                .max(Comparator.comparingLong(map::get));
        return maxUsername.orElse(DEFAULT_IF_EMPTY);
    }
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(final Student[] studentArray) {
        Predicate<Student> isCurrent = Student::checkIsCurrent;
        Predicate<Student> ageGreater = s -> s.getAge() > 20;
        Predicate<Student> gradeLess = s -> s.getGrade() < 65;
        return (int) Arrays.stream(studentArray).parallel()
                .filter(isCurrent.negate().and(ageGreater).and(gradeLess))
                .count();
    }
}
