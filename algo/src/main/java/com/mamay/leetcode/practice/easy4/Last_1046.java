...
public class Last_1046 {
    ...
    static long lights2(int n) {
        if (n == 1)
            return 1;
        return BigInteger.valueOf(2).pow(n).subtract(BigInteger.ONE).mod(BigInteger.valueOf(100000)).longValue();
    }
        ...
        for (int xi = 0; xi <= 50; xi++) {
            for (int yi = 0; yi <= 50; yi++) {
                ...
                for (int tower[] : towers) {
                    sum = sum + (int) (((double) tower[2]) / (1 + Math.sqrt((xi - tower[0]) * (xi - tower[0]) + (yi - tower[1]) * (yi - tower[1]))));
                }
                if (sum > maxSignal) {
