...
class MovieRentingSystem {
    final private Comparator<Entry> comparator = Comparator.comparingInt(Entry::getPrice)
    ...
    HashMap<Integer, HashMap<Integer, Integer>> movieToPrice = new HashMap<>(); // Map (shop, movie) -> price

    public MovieRentingSystem(int n, int[][] entries) {
        for (int[] entry : entries) {
            int shop = entry[0], movie = entry[1], price = entry[2];
            Entry movieEntry = new Entry(price, shop, movie);
            unrentedMap.computeIfAbsent(movie, x -> new TreeSet<>(comparator))
                    .add(movieEntry);
    ...
    public List<Integer> search(int movie) {
        return unrentedMap.getOrDefault(movie, Collections.emptySet()).stream()
                .limit(5).map(Entry::getShop).collect(Collectors.toList());
    }
