...
public class CheckEquivalence {
  ...
  public List<TreeNode> delNodes(TreeNode root, int[] to_delete) {
    HashSet<Integer> hashSet =
        Arrays.stream(to_delete).boxed().collect(Collectors.toCollection(HashSet::new));
    ...
    for (int i = 0; i < land.length; i++)
      for (int j = 0; j < land[0].length; j++)
        if (land[i][j] == 1 && (i == 0 || land[i - 1][j] == 0) && (j == 0 || land[i][j - 1] == 0)) {
          ...
          resultList.add(
              new int[] {i, j, i2 - 1, j2 - 1}); // we can have j = j2-1; after here as improvement
        }
    ...
    if (totalSumOfTree > 0)
      maxProduct =
          Math.max(
              maxProduct,
              Math.max(
                  leftSum * (totalSumOfTree - leftSum), rightSum * (totalSumOfTree - rightSum)));
  ...
  public String getDirections(TreeNode root, int startValue, int destValue) {
    ...
    int startLen = startPath.length,
        destLen = destPath.length,
        minLen = Math.min(startLen, destLen);
    int i = 0;
  ...
  public int lastRemaining(int n) {
    ArrayList<Integer> list =
        IntStream.rangeClosed(1, n).boxed().collect(Collectors.toCollection(ArrayList::new));
  ...
  private boolean find(TreeNode root, int val, StringBuilder builder) {
    ...
    if (root.left != null && find(root.left, val, builder)) builder.append("L");
    else if (root.right != null && find(root.right, val, builder)) builder.append("R");
  ...
  public String getDirectionsLeetcoded(TreeNode root, int startValue, int destValue) {
    ...
    int destLen = destBuilder.length(),
        startLen = startBuilder.length(),
        maxLen = Math.min(destLen, startLen),
        i;
    for (i = 0; i < maxLen; i++) {
  ...
  public int findRotateSteps(
      Map<Character, LinkedList<Integer>> map,
      String key,
      int currentIndex,
      int keyIndex,
      int ringLen) {
    ...
    for (int nextIndex : map.get(ch)) {
      ...
      minDistance =
          Math.min(
              minDistance, distance + findRotateSteps(map, key, nextIndex, keyIndex + 1, ringLen));
    }
    return minDistance;
  }
  ...
  public int numOfMinutes(int n, int headID, int[] manager, int[] informTime) {
    Map<Integer, List<Integer>> childMap =
        IntStream.range(0, n).boxed().collect(Collectors.groupingBy(i -> manager[i]));
  ...
  public int minimumTime(int[] hens, int[] grains) {
    ...
    while (low <= high) {
      ...
      if (minimumTimeBinarySearch(hens, grains, mid)) high = mid;
      else low = mid + 1;
    }
    return low;
  }
  ...
  public long[] distance(int[] nums) {
    ...
    Map<Integer, List<Integer>> map =
        IntStream.range(0, len).boxed().collect(Collectors.groupingBy(i -> nums[i]));
  ...
  public int maxSumDivThree(int[] nums) {
    ...
    listOfLists.add(
        Arrays.stream(nums)
            .filter(num -> num % 3 == 1)
            .boxed()
            .collect(Collectors.toCollection(LinkedList::new))); // 1
    listOfLists.add(
        Arrays.stream(nums)
            .filter(num -> num % 3 == 2)
            .boxed()
            .collect(Collectors.toCollection(LinkedList::new))); // 2
    int sum = Arrays.stream(nums).sum();
    List<Integer> remCountList =
        listOfLists.stream().map(subList -> subList.size() % 3).collect(Collectors.toList());
    final int minRemCount = remCountList.stream().min(Integer::compareTo).get();
    remCountList =
        remCountList.stream().map(remCount -> remCount - minRemCount).collect(Collectors.toList());
    boolean allNulls = remCountList.stream().allMatch(remCount -> remCount == 0);
    if (allNulls) return sum;
    int remIdx =
        IntStream.range(0, remCountList.size())
            .boxed()
            .min(Comparator.comparingInt(remCountList::get))
            .get();
    int remCount = remCountList.get(remIdx);
    int sum1 = listOfLists.get(remIdx).stream().sorted().limit(remCount).mapToInt(i -> i).sum();
    remCount = 3 - remCount;
    remIdx = 1 - remIdx;
    if (listOfLists.get(remIdx).size() < remCount) return sum - sum1;
    int sum2 = listOfLists.get(remIdx).stream().sorted().limit(remCount).mapToInt(i -> i).sum();
  ...
  public int maxTastiness(int[] price, int[] tastiness, int maxAmount, int maxCoupons) {
    ...
    // which is max tastiness at xth fruit with y amount of total money and z amount of total
    // coupons.
    ...
    for (int x = 1; x <= n; x++) { // iterate over n fruits (1 indexed for convinience).
      for (int y = 0; y <= maxAmount; y++) { // iterate over amount from 0 to maxAmount.
        for (int z = 0; z <= maxCoupons; z++) { // iterate over coupons from 0 to maxCoupon.
          ...
          // buy and take max of previous state i.e. previous fruit with less money and one less
          // coupon.
          if (z > 0 && y >= price[x - 1] / 2)
            dp[x][y][z] =
                Math.max(dp[x][y][z], tastiness[x - 1] + dp[x - 1][y - price[x - 1] / 2][z - 1]);
        }
      }
    }
  ...
  public List<String> removeInvalidParentheses(String s) {
    ...
    removeInvalidParentheses(
        s.toCharArray(), 0, rightBracketDeletions, leftBracketDeletions, new StringBuilder(), 0);
  ...
  private void removeInvalidParentheses(
      char[] arr,
      int idx,
      int rightBracketDeletions,
      int leftBracketDeletions,
    ...
    if (newCount >= 0) {
      ...
      removeInvalidParentheses(
          arr, idx + 1, rightBracketDeletions, leftBracketDeletions, builder, newCount);
      builder.deleteCharAt(builder.length());
    }
    if (arr[idx] == '(') {
      if (leftBracketDeletions > 0) {
        removeInvalidParentheses(
            arr, idx + 1, rightBracketDeletions, leftBracketDeletions - 1, builder, count);
      }
    } else {
      if (rightBracketDeletions > 0) {
        removeInvalidParentheses(
            arr, idx + 1, rightBracketDeletions - 1, leftBracketDeletions, builder, count);
      }
    }
  }
  ...
  public int minimumEffort(int[][] tasks) {
    ...
    Comparator<int[]> comparator =
        Comparator.comparing((int[] task) -> (double) task[1] / task[0])
