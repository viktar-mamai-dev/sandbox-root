...
public class FInd2323 {
    ...
    public int minNumber(int[] nums1, int[] nums2) {
        Arrays.sort(nums1);
        Arrays.sort(nums2);
        HashSet<Integer> set = Arrays.stream(nums2).boxed().collect(Collectors.toCollection(HashSet::new));
        for (int num : nums1) {
    ...
    private void findShortestCycle(LinkedList<LinkedList<Integer>> graph, int vertParent, int vert1,
                                   int[] used, HashMap<Integer, Integer> currentStack) {
        used[vert1] = 1;
    ...
    public long largestEvenSum(int[] nums, int k) {
        ...
        Map<Boolean, List<Integer>> minMap = Arrays.stream(nums, 0, len - k).boxed()
                .collect(Collectors.partitioningBy(evenPredicate));
        Map<Boolean, List<Integer>> maxMap = Arrays.stream(nums, len - k, len).boxed()
                .collect(Collectors.partitioningBy(evenPredicate));
    ...
    private static IntSummaryStatistics getStatistics(Map<Boolean, List<Integer>> minMap, boolean key) {
    ...
    public int maxFrequency(int[] nums, int k) {
        ...
        List<List<Integer>> list = map.keySet().stream()
                .map(key -> Arrays.asList(key, map.get(key))).collect(Collectors.toList());
    ...
    public int stoneGameVI(int[] aliceValues, int[] bobValues) {
        ...
        List<List<Integer>> list = IntStream.range(0, len)
                .mapToObj(i -> Arrays.asList(aliceValues[i], bobValues[i]))
                .collect(Collectors.toList());
        Collections.sort(list,
                Comparator.comparingInt(subList -> -Math.abs(subList.get(0) - subList.get(1))));
        int sum = IntStream.range(0, len)
                .map(i -> i % 2 == 0 ? list.get(i).get(0) : -list.get(i).get(1)).sum();
    ...
    public int leastInterval(char[] tasks, int n) {
        ...
        PriorityQueue<int[]> queue = IntStream.range(0, len)
                .mapToObj(i -> (tasks[i] - 'A'))
                .collect(Collectors.groupingBy(Function.identity(), Collectors.summingInt(x -> 1)))
                .entrySet().stream()
                .map(entry -> new int[]{entry.getKey(), entry.getValue()})
                .collect(Collectors.toCollection(() ->
                        new PriorityQueue<>(Comparator.comparingInt((int[] a) -> a[1]).reversed())));
        while (!queue.isEmpty()) {
    ...
    public long minSumSquareDiff(int[] nums1, int[] nums2, int k1, int k2) {
        PriorityQueue<int[]> q = new PriorityQueue<>((int[] a1, int[] a2) -> Integer.compare(a2[0], a1[0]));
        int n = nums1.length;
        for (int i = 0; i < n; i++) {
    ...
    private void main() {
        ...
        // populate source map
        Map<String, List<Object2>> targetMap = sourceMap.entrySet().stream()
                .collect(Collectors.groupingBy(entry -> entry.getKey().getValue(),
                        Collectors.mapping(Map.Entry::getValue, Collectors.toList())));

        Map<String, List<Object2>> targetMap2 = sourceMap.keySet().stream()
                .collect(Collectors.groupingBy(Object1::getValue,
                        Collectors.mapping(sourceMap::get, Collectors.toList())));
    }
    ...
    public int[] smallestTrimmedNumbers(String[] nums, int[][] queries) {
        ...
        Comparator<StringNode> comparator = Comparator.comparing(
                        (StringNode s) -> s.str.substring(s.trimmedPrefix))
    ...
    public List<String> mostVisitedPattern(String[] username, int[] timestamp, String[] website) {
            ...
            for (int i = 0; i < len - 2; i++) {
                for (int i2 = i + 1; i2 < len - 1; i2++) {
                    for (int i3 = i2 + 1; i3 < len; i3++) {
                        StringTriplet s = new StringTriplet(timeToVisit.get(i), timeToVisit.get(i2),
                                timeToVisit.get(i3));
                        set.add(s);
                    }
                }
        ...
        Map<StringTriplet, Long> tripletToFreq = tripletSetList.stream()
                .flatMap(Collection::stream)
                .collect(Collectors.groupingBy(t -> t, Collectors.counting()));
        Comparator<StringTriplet> comparator = Comparator.comparing((StringTriplet s) -> -tripletToFreq.get(s))
                .thenComparing((StringTriplet s) -> s.s1)
                .thenComparing((StringTriplet s) -> s.s2)
                .thenComparing((StringTriplet s) -> s.s3).reversed();
    ...
    public int wordCount(String[] startWords, String[] targetWords) {
        ...
        HashSet<ArrayWrapper> set = Arrays.stream(startWords).map(s -> new ArrayWrapper(parse(s)))
    ...
    private boolean isPair(int[] ranks) {
        HashSet<Integer> set = Arrays.stream(ranks).boxed().collect(Collectors.toCollection(HashSet::new));
        return set.size() < 5;
    }

    private boolean isFlush(char[] suits) {
        return suits[0] == suits[1] && suits[2] == suits[3] && suits[0] == suits[4] && suits[0] == suits[2];
    }
    ...
    public List<String> wordBreak(String s, List<String> wordDict) {
        ...
        IntSummaryStatistics statistics = wordDict.stream().mapToInt(w -> w.length()).summaryStatistics();
    ...
    private boolean isCheck(HashMap<Integer, Integer> map) {
        ...
        return (key1 - 1 == key2 && val1 == 1) || (key2 - 1 == key1 && val2 == 1) ||
                (key1 == 1 && key2 == 1) || (key1 == 1 && val1 == 1) || (key2 == 1 && val2 == 1);
    }
    ...
    public int minimumOperations(int[] nums) {
        return Arrays.stream(nums).filter(x -> x != 0).boxed()
    ...
    public long[] minimumCosts(int[] regular, int[] express, int expressCost) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            dp[i][1] = Math.min(dp[i - 1][0] + regular[i - 1] + expressCost, dp[i - 1][1] + express[i - 1]);
        }
    ...
    public List<String> alertNames(String[] keyName, String[] keyTime) {
        ...
        return map.entrySet().stream().filter(entry -> isAlerted(entry.getValue()))
    ...
    public int numberOfWays(int startPos, int endPos, int k) {
        ...
        BigInteger res = factorialHavingLargeResult(k)
    ...
    public int countDaysTogether(String arriveAlice, String leaveAlice, String arriveBob, String leaveBob) {
        ...
        return (int) retrieveMin(minDate(aAlicaDate, lBobDate), minDate(aBobDate, lAlicaDate),
                minDate(aAlicaDate, lAlicaDate), minDate(aBobDate, lBobDate));
    }
    ...
    private LocalDate extractDate(String strDate) {
        return LocalDate.of(2021, Integer.parseInt(strDate.substring(0, 2)),
    ...
    public int maxProduct(String[] words) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            setArray[i] = IntStream.range(0, chArray.length).mapToObj(j -> chArray[j])
                    .collect(Collectors.toCollection(HashSet::new));
        }
        int max = 0;
    ...
    private long getWidth(String text, int font, FontInfo fontInfo) {
        return IntStream.range(0, text.length())
                .mapToLong(i -> fontInfo.getWidth(font, text.charAt(i))).sum();
    }
    ...
    public int[] assignBikes(int[][] workers, int[][] bikes) {
        ...
        Comparator<WorkerBike> comparator = Comparator.comparingInt(WorkerBike::getDist)
    ...
    public int findLongestChain(int[][] pairs) {
        Arrays.sort(pairs, Comparator.comparingInt((int[] a) -> a[1]).thenComparingInt((int[] a) -> a[0]));
    ...
    public int maxDistance(int[][] grid) {
        ...
        return (int) IntStream.range(0, m)
                .mapToLong(i -> IntStream.range(0, n).mapToLong(j -> grid[i][j])
                        .max().getAsLong()
                ).max().getAsLong() - 1;
    }
    ...
    private void enrich(HashMap<String, Integer> map, HashMap<String, Integer> subMap, int multiplier) {
    ...
    public long interchangeableRectangles(int[][] rectangles) {
        Map<RectPair, Long> map = Arrays.stream(rectangles).map(convertToPair())
                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
    ...
    public List<String> invalidTransactions(String[] transactions) {
        final Comparator<Transaction> comparator =
                Comparator.comparingInt(Transaction::getTime)
                        .thenComparingInt(Transaction::getIndex);
        int len = transactions.length;
        Map<String, TreeSet<Transaction>> map = IntStream.range(0, len)
    ...
    private Transaction parseTransaction(String[] transactions, int index) {
        ...
        return new Transaction(arr[0], arr[3], Integer.parseInt(arr[1]),
                Integer.parseInt(arr[2]), index);
    }
    ...
    public int waysToSplit(int[] nums) {
        ...
        for (int i = 1; i < length - 1; ++i) {
            if (prefixSum[i - 1] > (prefixSum[length - 1] - prefixSum[i - 1]) / 2) break;  // early termination
    ...
    public int findSmallestInteger(int[] nums, int value) {
        Map<Integer, Long> map = Arrays.stream(nums).boxed().map(num -> num % value)
                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
    ...
    class FoodRatings {
        ...
        private final HashMap<String, TreeMap<Integer, TreeSet<String>>> cuisineToRating = new HashMap<>();
        ...
        public FoodRatings(String[] foods, String[] cuisines, int[] ratings) {
            ...
            for (int i = 0; i < len; i++) {
                ...
                TreeMap<Integer, TreeSet<String>> treeMap = cuisineToRating.getOrDefault(cuisines[i], new TreeMap<>());
        ...
        public void changeRating(String food, int newRating) {
            ...
            TreeMap<Integer, TreeSet<String>> treeMap = cuisineToRating.getOrDefault(cuisine, new TreeMap<>());
